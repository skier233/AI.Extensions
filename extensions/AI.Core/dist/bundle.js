...
import ReactDOMClient from "@cove/runtime/react-dom-client";
...
function collectCapabilityFeatures(capabilities, mediaKind, customPipelines = []) {
...
function collectSelectedClaimIds(features, claims, selectedCapabilityIds) {
  ...
  if (claimIds.size === 0) {
    (claims || [])
      .filter((claim) => selected.has(claim.capabilityId))
      .forEach((claim) => claimIds.add(claim.claimId));
  }
...
function getSlotModelCandidates(catalog, slot) {
  ...
  (slot?.defaultModels || []).forEach((modelName) => {
    if (!models.some((model) => model.configName.toLowerCase() === modelName.toLowerCase())) {
      ...
      models.push({
        ...
        pinned: !!catalogModel?.pinned,
        active: catalogModel?.active !== false,
        unavailable: !catalogModel,
      });
    }
  });
  ...
  return (capabilities || []).flatMap((descriptor) => (descriptor.capabilities || []).flatMap((feature) => (
    (feature.modelBindingSlots || [])
      ...
      .map((slot) => {
        ...
        const defaultModel = (slot.defaultModels || [])[0] || models[0]?.configName || "";
        return {
...
function collectRequestedTaggingModels(catalog, scope, selectedTaggingCategories, capabilityModelBindings) {
  ...
  (catalog.models || []).forEach((model) => {
    if (!isTaggingModel(model) || !getTaggingScopes(model).includes(resolvedScope)) {
      return;
    }
...
function collectTaggingPreferenceRows(catalog, preferences) {
  ...
  return Array.from(rows.values())
    .map((row) => {
      ...
      const resolvedModels = selectedModelUnavailable
        ? [...models, { configName: selectedModel, loaded: undefined, pinned: false, active: false, unavailable: true }]
        : models;
...
function collectTaggingCategories(catalog, mediaKind) {
  ...
  (catalog.models || []).forEach((model) => {
    if (!isTaggingModel(model) || !getTaggingScopes(model).includes(requiredScope)) {
      return;
    }
...
function collectRequestedModels(catalog, claims, mediaKind, selectedClaimIds, selectedTaggingCategories, capabilityModelBindings) {
  ...
  (claims || []).forEach((claim) => {
    ...
    if (binding?.model) {
      ...
      requested.set(binding.model, {
        ...
        loaded: catalogModel?.loaded === true ? true : catalogModel?.loaded === false ? false : undefined,
        pinned: catalogModel ? !!catalogModel.pinned : undefined,
      });
      return;
    }
...
function copySettings(settings) {
  return {
    ...
    defaultLoadPolicy: settings?.defaultLoadPolicy || "load_or_fail",
...
function SettingsEditor({ settings, health, catalog, capabilities = [], busy, message, pipelineBusy = "", onChange, onSave, onSyncPipeline = null, onDeletePipeline = null }) {
  const mappings = settings.pathMappings || [];
  const modelBindingRows = getModelBindingRows(capabilities, catalog, settings);
  const taggingPreferenceRows = collectTaggingPreferenceRows(catalog, settings.capabilityModelBindings);
  ...
  return h("div", { className: "ai-core-stack" }, [
    ...
    h("div", { className: "ai-core-section", key: "taggingPreferences" }, [
      h("div", { className: "ai-core-section-header" }, [
        h("div", null, [
          h("h4", { className: "ai-core-subtitle" }, "Content tagging category bindings"),
          h("p", { className: "ai-core-muted" }, "New tagging categories come from the model catalog automatically; no Cove extension code changes are needed."),
        ]),
      ]),
      taggingPreferenceRows.length === 0
        ? h("p", { className: "ai-core-empty" }, (settings.capabilityModelBindings || []).some((binding) => binding.capabilityId === "tagging")
            ? "Saved tagging bindings will appear here when the model catalog is reachable again."
            : "No tagging categories are available from the current model catalog.")
        : h("div", { className: "ai-core-claim-list" }, taggingPreferenceRows.map((row) => h("div", { className: "ai-core-model-row", key: row.key }, [
            h("div", { className: "ai-core-model-copy" }, [
              h("div", { className: "ai-core-model-title" }, row.category),
              h("div", { className: "ai-core-model-meta" }, [
                h("span", { className: "ai-core-pill ai-core-pill-muted", key: "scope" }, row.scope),
                row.defaultModel
                  ? h("span", { key: "default" }, `Auto -> ${row.defaultModel.configName}`)
                  : h("span", { key: "default" }, "No fallback candidate available"),
                row.selectedModelUnavailable
                  ? h("span", { className: "ai-core-pill ai-core-pill-warning", key: "missing" }, "saved model unavailable")
                  : null,
              ]),
            ]),
            h("label", { className: "ai-core-field ai-core-preference-field" }, [
              ...
              h("select", {
                className: "ai-core-select ai-core-preference-select",
                value: row.selectedModel || "",
                onChange: (event) => patchTaggingPreference(row.scope, row.category, event.target.value),
              }, [
                h("option", { key: "auto", value: "" }, row.defaultModel ? `Auto (${row.defaultModel.configName})` : "Auto"),
                ...row.models.map((model) => h("option", { key: model.configName, value: model.configName }, formatTaggingModelOptionLabel(model))),
              ]),
            ]),
          ]))),
...
function ModelsPanel({ catalog, busyModel, onLoadToggle, onPinToggle, onRefresh }) {
  const groups = groupModels(catalog?.models || []);

  const groupSections = groups.map((group) => {
    const modelRows = group.items.map((model) => {
      const busy = busyModel === model.configName;
      return h("div", { className: "ai-core-model-row", key: model.configName }, [
        h("div", { className: "ai-core-model-copy" }, [
          h("div", { className: "ai-core-model-title" }, model.name || model.configName),
          h("div", { className: "ai-core-model-meta" }, [
            h("span", { className: "ai-core-code" }, model.configName),
            model.version ? h("span", { className: "ai-core-pill ai-core-pill-muted", key: "version" }, model.version) : null,
            model.loaded ? h("span", { className: "ai-core-pill ai-core-pill-success", key: "loaded" }, "loaded") : null,
            model.pinned ? h("span", { className: "ai-core-pill ai-core-pill-warning", key: "pinned" }, "pinned") : null,
          ]),
        ]),
        h("div", { className: "ai-core-model-actions" }, [
          h("label", { className: "ai-core-inline-checkbox", key: "pin" }, [
            h("input", {
              className: "ai-core-checkbox",
              type: "checkbox",
              checked: !!model.pinned,
              disabled: busy,
              onChange: (event) => onPinToggle(model.configName, event.target.checked),
            }),
            h("span", null, "Pin"),
          ]),
          h("button", {
            ...
            type: "button",
            disabled: busy,
            onClick: () => onLoadToggle(model.configName, !model.loaded),
          }, busy ? "Working..." : model.loaded ? "Unload" : "Load"),
        ]),
      ]);
    });

    return h("section", { className: "ai-core-model-group", key: group.groupKey }, [
      h("div", { className: "ai-core-model-group-header" }, group.groupKey),
      h("div", { className: "ai-core-stack ai-core-tight" }, modelRows),
    ]);
  });
  ...
  return h("div", { className: "ai-core-stack" }, [
    h("div", { className: "ai-core-section-header", key: "header" }, [
      h("div", null, [
        ...
        h("p", { className: "ai-core-muted" }, "Load, unload, and pin the model set exposed by nsfw_ai_server v4."),
      ]),
      h("button", {
        className: "ai-core-button ai-core-button-secondary",
        type: "button",
    ...
    groups.length === 0
      ...
      : h("div", { className: "ai-core-stack", key: "groups" }, groupSections),
  ]);
}
...
function RunComposer({ capabilities, catalog, settings, busy, message, onQueue, selection = null, submitLabel = "Queue AI job" }) {
  ...
  const initialFeatures = collectCapabilityFeatures(capabilities, initialMediaKind, settings?.customPipelines || []);
  ...
  const [form, setForm] = useState({
    ...
    capabilityIds: hasSavedCapabilityIds
      ? savedDefaults.capabilityIds.filter((capabilityId) => typeof capabilityId === "string")
      : initialFeatures.map((feature) => feature.capabilityId),
  ...
  const features = collectCapabilityFeatures(capabilities, form.mediaKind, settings?.customPipelines || []);
  ...
  const requestedModels = collectRequestedModels(catalog, claims, form.mediaKind, selectedClaimIds, effectiveTaggingCategories, settings?.capabilityModelBindings || []);
  ...
  function changeMediaKind(mediaKind) {
    const nextFeatures = collectCapabilityFeatures(capabilities, mediaKind, settings?.customPipelines || []);
    ...
    h("div", { className: "ai-core-run-summary rounded-xl border border-border bg-card p-4", key: "modelSummary" }, [
      h("div", { className: "ai-core-section-header" }, [
        h("div", null, [
          ...
          h("p", { className: "ai-core-muted" }, summarizeRequestedModels(requestedModels, settings.defaultLoadPolicy || "load_or_fail")),
        ]),
      ]),
      h("div", { className: "ai-core-toolbar" }, [
      ...
      requestedModels.length > 0
        ? h("div", { className: "ai-core-model-chip-list" }, requestedModels.map((model) => h("div", { className: "ai-core-model-chip rounded-full border border-border bg-surface text-foreground", key: model.configName }, [
            ...
            model.pinned ? h("span", { className: "ai-core-pill ai-core-pill-warning" }, "pinned") : null,
      ...
      h("p", { className: "ai-core-muted" }, "Use the AI page's Loaded model set section when you want to load, unload, or pin models ahead of a run."),
    ]),
...
function AiCorePage() {
  ...
  async function togglePin(configName, pinned) {
    setBusyModel(configName);
    try {
      await api("/models/pin", {
        method: "POST",
        body: JSON.stringify({ models: [configName], pinned }),
      });
      await refresh();
    } catch (error) {
      setSettingsMessage(error.message || "Failed to update model pinning.");
    } finally {
      setBusyModel("");
    }
  }
  ...
  return h("div", { className: "ai-core-shell" }, [
    ...
    h("section", { className: "ai-core-card", key: "models" }, [
      h("div", { className: "ai-core-card-header" }, [
        h("div", null, [
          h("h3", { className: "ai-core-title" }, "Loaded model set"),
          h("p", { className: "ai-core-muted" }, "The active model set lives on the server. AI Core keeps the picker here instead of hiding it behind config files or restarts."),
        ]),
      ]),
